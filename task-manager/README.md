...
This app lives in `task-manager/` inside the broader `productivity` git repository.
...
Run these from the `task-manager/` directory:
...
- Source files under `task-manager/` are intended to be tracked by git
- Build output, dependencies, env files, csv files, and xlsx files are ignored
- Git commands can be run from the repo root or from this directory
- Typical workflow: `git status`, `git add task-manager`, `git commit`
